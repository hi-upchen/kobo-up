  ...
  describe('arrayBufferToBase64', () => {
    test('should handle large arrays without stack overflow', () => {
      ...
      expect(() => {
        ...
        const result = (KoboService as any).arrayBufferToBase64(largeArray)
    ...
    test('should produce valid base64 output', () => {
      ...
      const result = (KoboService as any).arrayBufferToBase64(testArray)
      expect(result).toBe('SGVsbG8=') // Base64 for "Hello"
    })

    test('should handle empty array', () => {
      const emptyArray = new Uint8Array(0)
      const result = (KoboService as any).arrayBufferToBase64(emptyArray)
      expect(result).toBe('')
    })
